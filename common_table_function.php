...
{incorrect-> {2xd , y2}"
	...
	echo '<tr><td class=note>Action --></td><td><button class=button type=submit name=action 
...
//show search result based on sql
function show_sql($link,$d,$t,$sql)
{
	//my_print_r($_POST);
...
function verify_ap_user($du,$dp,$role,$ud,$ut,$uf,$uv,$pf,$pv)
{
	...
	if(!password_verify($pv,$result_array['password'])){echo 'Application user not verified';return false;}

	return true;
}


///Row level permission of three types: none,read,edit,delete 0,1,2,3
///insert level permission governed by role - because row donot exist
///incomplate function
function verify_row_permission($link,$d,$t,$u,$pk_value_array)
{
	$sql=mk_select_sql_from_pk($link,$d,$t,$pk_value_array);
	$result=run_query($link,$d,$sql);
	$data=get_single_row($result);
	$allowed=explode('|',$data['rights']);
	foreach ($allowed as $v)
	{
		$v_data=explode('^',$v);
		if(isset($v_data[0]) && isset($v_data[1]) )
		{
